    ...
    def __init__(self, args, transformer=None, parallel_output=True, **kw_args):
        ...
        if args.use_lora:
            self.add_mixin(
                "lora",
                LoraMixin(
                    args.num_layers, args.lora_rank, layer_range=args.layer_range
                ),
                reinit=True,
            )
...
class MMMDDataset(Dataset):
    def __init__(self, path, processor, image_size, tokenizer, args):
        ...
        dummy_img = processor(Image.new("RGB", (1, 1)))
        img_cache = {}
        ...
        for item in tqdm(data):
            if item["img"]:
                if item["img"] not in img_cache:
                    image = processor(
                        Image.open(args.img_base_dir + "/" + item["img"]).convert("RGB")
                    )
                    img_cache[item["img"]] = image
                else:
                    image = img_cache[item["img"]]
                ...
                pre_image = max_seq_length + 10  # to ignore the image
            ...
            self.pre_image.append(pre_image)
...
def create_dataset_function(path, args):
    ...
    image_size = 224
    image_processor = BlipImageEvalProcessor(image_size)

    dataset = MMMDDataset(path, image_processor, image_size, tokenizer, args)
    return dataset
...
if __name__ == "__main__":
    ...
    py_parser.add_argument("--img_base_dir", type=str)
    ...
    def data_collator(examples):
        ...
        ret = {
            ...
            "pre_image": [example["pre_image"] for example in examples],
        }
        return ret
