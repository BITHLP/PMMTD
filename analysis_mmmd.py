...
def decode_mmmd(model, tokenizer, example, yes_token, no_token, eol_token, args):
    ...
    for field in ("当前目标", "当前实体", "剩余回合", "回复图片"):
        prefix += field + ": "
        prefix += (example[field] or "") + "\n"
        predicted[field] = example[field] or ""
...
def remove_knowledge(prompt: str):
    r = prompt.find("### 对话:")
    return "### 知识:\n\n" + prompt[r:]
...
def main():
    ...
    parser.add_argument("--mask_field", type=str)
    ...
    # args.mask_field = "当前目标"  # "input_image", "当前目标", "当前实体", "剩余回合", "回复图片","structure","knowledge"
    print(args.mask_field)
    args.result_file = f"./eval/analysis-{args.mask_field}.jsonl"
    ...
    with open(args.result_file, "w") as f:
        for example in tqdm(examples):
            if args.mask_field == "structure":
                for fld in ("当前目标", "当前实体", "剩余回合", "回复图片"):
                    example[fld] = None
            elif args.mask_field == "knowledge":
                example["prompt"] = remove_knowledge(example["prompt"])
            else:
                example[args.mask_field] = None
